        	...
        	if (req->times_sent >= 5) {
            	struct sr_packet * curr_packet  = req->packets;          
           		while (curr_packet != NULL) {
                        struct sr_ip_hdr * ip_hdr  = (struct sr_ip_hdr *)(curr_packet + sizeof(struct sr_ethernet_hdr));
                        struct sr_rt * rt = lpm(sr , ip_hdr -> ip_src);
                        if(rt){
                            /* Send type 3 code 1 ICMP (Host Unreachable) */
                            send_icmp_three_packet(sr, curr_packet->buf, rt->interface, curr_packet->len, sr_icmp_type_three, sr_icmp_code_one);
                        }
                        else{
                ...
                struct sr_ethernet_hdr * reply_eth_hdr = (struct sr_ethernet_hdr *)request_packet;
                struct sr_arp_hdr *reply_arp_hdr = ((struct sr_arp_hdr *)(request_packet + sizeof(struct sr_ethernet_hdr)));
                ...
                memcpy(reply_arp_hdr -> ar_tha,broadcast_addr,ETHER_ADDR_LEN);		
    			reply_eth_hdr->ether_type = htons(ethertype_arp);
                memcpy(reply_eth_hdr -> ether_shost,interface->addr,ETHER_ADDR_LEN);
                memcpy(reply_eth_hdr -> ether_dhost,broadcast_addr,ETHER_ADDR_LEN);
